...
public class Solution {
	...
	class Printer implements Comparable<Printer>{
		private int prio;
		private int number;
		Printer(int prio,int number){
			this.prio = prio;
			this.number = number;
		}
		@Override
		public int compareTo(Printer p) {	//음수 or 0이면 객체의 자리가 그대로 유지되며, 양수일경우 두객체 자리바꿈
			if(this.prio < p.prio) {
				return 1;
			}
			else if(this.prio > p.prio) {
				return -1;
			} else {
				return 0;
			}
	 ...
	 public int solution(int[] priorities, int location) {
		...
		for(int i =0 ; i < priorities.length; i++ ) {
			pq.offer(new Printer(priorities[i],i));
		}
		while(!pq.isEmpty()) {
			cnt++;
			Printer pr = pq.poll();
			if(pr.number == location) {
				break;
			}
